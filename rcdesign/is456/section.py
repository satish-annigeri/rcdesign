...
class RectBeamSection(Section):
    ...
    def report(self, xu: float, ecu: float):  # pragma: no cover
        ...
        console = Console()
        console.print(
            f"[bold blue]Rectangular Beam Section[/bold blue] {self.b} x {self.D}  (xu = {xu:.2f})"
        )
        print(
    ...
    def sv(self, Vu: float, nlegs: int, bar_dia: int, mof: float = 25):
        ...
        Vus = Vu - Vuc
        self._sv = (
            self.shear_steel.rebar.fd * self.shear_steel._Asv * self.eff_d() / Vus
        )
    ...
    def report(self, xu: float, ecu: float):  # pragma: no cover
        ...
        if self.c_steel:
            ...
            for layer in self.c_steel.layers:
                ...
                Mw += Msc
                print(
                    f"{layer.dc:4.0f} {layer.area:8.2f} {x:8.2f} {esc:12.4e} {fsc:8.2f} {fcc:8.2f} {Fsc/1e3:8.2f} {Msc/1e6:8.2f}"
                )
        else:
            print(f"{'-':>37} {'-':>8}")
        print("-" * 71)
        ...
        Mt = 0
        print(
            f"{'dc':>4} {'Area':>8} {'x':>8} {'Strain':>12} {'f_st':>8} {' ':8} {'T (kN)':>8} {'M (kNm)':>8}"
        )
        for layer in self.t_steel.layers:
            x = self.D - xu - layer.dc
            est = ecu / xu * x
            ...
            Mt += Mst
            print(
                f"{layer.dc:4.0f} {layer.area:8.2f} {x:8.2f} {est:12.4e} {fst:8.2f} {' ':8} {Fst/1e3:8.2f} {Mst/1e6:8.2f}"
            )
