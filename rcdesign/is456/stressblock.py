...
class LSMStressBlock:
    def __init__(
        self, label: str = "LSM Stress Block", ecy: float = ecy, ecu: float = ecu
    ):
        self.label = label
        self.ecy = ecy
        self.ecu = ecu
    ...
    def _fc(self, _z: float, _k: float, ecmax: float = ecu) -> Mul:
        ecmax = self.isvalid_ecmax(ecmax)
        # z, k = symbols("z k")
        if _k < 0:  # Invalid values for k
            raise ValueError
            ...
            ec = self.ec(_z, _k, ecmax)
            if ec < 0:
                fc = nsimplify(0)
            elif ec < 1:
                _ec = self._ec(_k, ecmax)
                fc = (2 * _ec - _ec ** 2).evalf(subs={"k": _k})
            else:
                fc = nsimplify(1)
        return fc
    ...
    def fc(self, z: float, k: float, ecmax: float = ecu) -> float:
        ...
        return fc.evalf(subs={"z": z})

    def C(self, z1: float, z2: float, _k: float, ecmax: float = ecu) -> float:
        ecmax = self.isvalid_ecmax(ecmax)
        z, k = symbols("z k")
        _k = self.isvalid_k(_k)
        if _k == 0:
            return 0.0

        z1 = self.isvalid_z(z1, _k)
        z2 = self.isvalid_z(z2, _k)
        if z1 > z2:
            z1, z2 = z2, z1
        ec1 = self.ec(z1, _k)
        ec2 = self.ec(z2, _k)
        if ec2 <= 1:  # Parabolic only
            fcp = self._fc(z1, _k, ecmax)
            Cp = integrate(fcp, (z, z1, z2))
            Cr = 0.0
        elif ec1 >= 1:  # Rectangular only
            Cp = 0.0
            fcr = self._fc(z1, _k, ecmax)
            Cr = integrate(fcr, (z, z1, z2))
        else:  # Both Parabolic and Rectangular
            if _k <= 1:  # NA within the section
                zz = self.ecy / ecmax * _k
            else:  # NA outside the section
                zz = _k - (1 - self.ecy / self.ecu)
            fcp = self._fc(z1, _k, ecmax)
            fcr = self._fc(z2, _k, ecmax)
            Cp = integrate(fcp, (z, z1, zz))
            Cr = integrate(fcr, (z, zz, z2))
        return Cp + Cr

    def M(self, z1: float, z2: float, _k: float, ecmax: float = ecu) -> float:
        ecmax = self.isvalid_ecmax(ecmax)
        z, k = symbols("z k")
        _k = self.isvalid_k(_k)
        if _k == 0:
